...
bool serialize(rapidxml::xml_node<> * node, const std::pair<Key, Value> & in, rapidxml::xml_document<> & doc)
{
    auto node1 = doc.allocate_node(rapidxml::node_element, "first");
    node->append_node(node1);
    if(!serialize(node1, in.first, doc))
        return false;

    auto node2 = doc.allocate_node(rapidxml::node_element, "second");
    node->append_node(node2);
    if(!serialize(node2, in.second, doc))
        return false;
    return true;
}
...
//map
template<typename Key, typename Value>
bool serialize(rapidxml::xml_node<> * node, const std::map<Key, Value> & in, rapidxml::xml_document<> & doc)
{
    for(auto & i : in)
    {
        auto node1 = doc.allocate_node(rapidxml::node_element, "item");
...
//pair
template<typename Key, typename Value>
bool unserialize(const rapidxml::xml_node<> * node, std::pair<Key, Value> & out)
{
    if(!node)
        return false;
...
//map
template<typename Key, typename Value>
bool unserialize(const rapidxml::xml_node<> * node, std::map<Key, Value> & out)
{
    if(!node)
        return false;
