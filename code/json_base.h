...
bool serialize(rapidjson::Value & val, const std::pair<Key, Value> & in, rapidjson::Document & doc)
{
    rapidjson::Value v(rapidjson::kObjectType);

    rapidjson::Value v1;
    if(!serialize(v1, in.first, doc))
        return false;
    v.AddMember("first", std::move(v1), doc.GetAllocator());

    rapidjson::Value v2;
    if(!serialize(v2, in.second, doc))
        return false;
...
//map
template<typename Key, typename Value>
bool serialize(rapidjson::Value & val, const std::map<Key, Value> & in, rapidjson::Document & doc)
{
    rapidjson::Value v(rapidjson::kArrayType);
    for(auto &i : in)
    {
...
//pair
template<typename Key, typename Value>
bool unserialize(const rapidjson::Value & val, std::pair<Key, Value> & out)
{
    if(!val.IsObject())
        return false;

    if(!unserialize(val["first"], out.first))
        return false;

    if(!unserialize(val["second"], out.second))
        return false;
...
//map
template<typename Key, typename Value>
bool unserialize(const rapidjson::Value & val, std::map<Key, Value> & out)
{
    if(!val.IsArray())
        return false;
